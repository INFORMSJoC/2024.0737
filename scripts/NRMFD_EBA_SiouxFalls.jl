...
@variable(prob, eta)
@constraint(prob, - y - d - eta * ones(ny) <= 0)
...
function NRMFD_EBA_SF(; name = "SiouxFalls", jam=1., maxIter = 50, verbose = 2, results_root = "../results/CNDP/")
...
for nIter in 1:maxIter
    ...
    while decent_flag
        ...
        # if length(alphas) >= 20; decent_flag = false | (name == "SiouxFalls"); end
    ...
    for alpha in reverse(alphas)
        ...
        if upper_obj(td, flow0, ep0) < upper_obj(td, flow, ep)
            ...
            # if verbose > 0; 
                @printf("success update with alpha: %.2e\n", alpha)
            # end
            break
        end
...
CSV.write(results_root*name*"_EBA_y_jam_$(jam).csv", DataFrame(ys, :auto))
CSV.write(results_root*name*"_EBA_jam_$(jam).csv", df0)
